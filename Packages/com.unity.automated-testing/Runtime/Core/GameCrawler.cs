...
namespace Unity.AutomatedQA
{
    public class GameCrawler : MonoBehaviour
    {
        ...
        public void Initialize(GameCrawlerAutomatorConfig Config)
        {
            Stop = false;
            this.Config = Config;
            isHighlightActivated = AutomatedQARuntimeSettings.ActivateHighlightFeedbackFx;
            AutomatedQARuntimeSettings.ActivateHighlightFeedbackFx = false; // TODO: We need to rework how we perform drags, or else there will be a lot of messy code to perform these without saving the interpolated events.
        ...
        private IEnumerator Crawl()
        {
            ...
            AutomatedQARuntimeSettings.ActivateHighlightFeedbackFx = isHighlightActivated;
            IsCrawling = false;

            // Do not generate a report for a run that lasted too long (over half an hour). All actions taken will still be logged normally.
            if (GetAdjustedTime() < Config.SecondsToRunBeforeSkippingGenerationOfAReport)
        ...
        private List<RecordingInputModule.TouchData> DetermineBestTouchActionsToPerform(AutomationListener al)
        {
            ...
            td.timeDelta = GetAdjustedTime() - LastRecordedAction.time;
            ...
            else if (IsDropDownSelectionInProgress.isInProgress && LastRecordedAction.touch.eventType == RecordingInputModule.TouchData.type.release)
            {
                ...
                td.objectHierarchy = string.Join("/", RecordingInputModule.Instance.GetHierarchy(targetOption));
        ...
        private RecordingInputModule.TouchData GetBasicTouchData(AutomationListener al)
        {
            ...
            {
                objectName = al.name,
                objectHierarchy = string.Join("/", RecordingInputModule.Instance.GetHierarchy(al.gameObject)),
                timeDelta = 1f,
                inputDuration = 0.1f,
                positional = false,
