...
namespace Unity.AutomatedQA.Editor
{
    public static class CodeGenerator
    {
        ...
        public static List<(string recordingFileName, string cSharpScriptFileName, bool isStepFile)> GenerateTestFromAutomatedRun(string automatedRunFileName, bool ignoreDiffs, bool isSimpleTest, string stepFileToOverwrite = "")
        {
            ...
            string className = $"{GetClassNameForRecording(automatedRunFileName)}_Tests";
            ...
            foreach (AutomatorConfig automatedConfig in automatedRun.config.automators)
            {
                ...
                {
                    touchDataList.AppendLine($"{indentThree}yield return RunAutomator(typeof({automatedConfig.AutomatorType}), {automatorIndex++});");
                    continue;
                }
                ...
                stepFileClassName = lastJsonFileName + "_Steps";
                ...
                {
                    NestedSegmentHeirarchy = (assetFilePath, new List<string>());
                ...
                foreach ((string file, TouchData data) touchData in steps)
                {
                    ...
                    */
                    setUpLogic.AppendLine($"{indentThree}Driver.Perform.RegisterStep({stepFileClassName}.Actions[\"{touchIdFinal}\"]);");
                        string testStep = $"{indentThree}yield return Driver.Perform.Action({stepFileClassName}.Actions[\"{touchIdFinal}\"]); " + (touchData.data.eventType == TouchData.type.none ? $"// Emit {touchData.data.emitSignal}" :
            ...
            fullScript.AppendLine("using Unity.AutomatedQA;");
            fullScript.AppendLine("using Unity.CloudTesting;");
            fullScript.AppendLine($"{NEW_LINE}namespace GeneratedAutomationTests");
            ...
            fullScript.AppendLine($"{indentOne}{{{NEW_LINE}"); // Start class bracket.
            fullScript.AppendLine($"{indentTwo}/// Generated from recording file: \"{automatedRunFileName}\".{NEW_LINE}" +
            ...
            fullScript.AppendLine($"{indentTwo}// Initialize test run data.{NEW_LINE}" +
                                  ...
                                  (recordingToUseForBaselineAspectAndResolution != null?
                                     $"{indentThree}Test.entryScene = \"{recordingToUseForBaselineAspectAndResolution.entryScene}\";{NEW_LINE}" +
            ...
            {
                GenerateAutomatedRunTest(automatedRunFileName);
            }
            return filesWithEdits;
        }

        public static List<(string recordingFileName, string cSharpScriptFileName, bool isStepFile)> GenerateTestFromRecording(string recordingFileName, bool ignoreDiffs, bool isSimpleTest, string stepFileToOverwrite = "")
        {
            // Create required files for UnityTest script creation.
            CreateTestAssemblyFolder();
            CreateTestAssembly();
        ...
        private static void CreateStepFile(string stepFileRecording, string stepFileCSharpName, StringBuilder stepFile, bool ignoreDiffs, string stepFileToOverwrite, ref List<(string recordingFileName, string cSharpScriptFileName, bool isStepFile)> filesWithEdits)
        {
            ...
            else if (!File.Exists(stepFilePath) || (ignoreDiffs && stepFileToOverwrite.Replace(".json", string.Empty) == stepFileRecording.Replace(".json", string.Empty)))
            {
        ...
        public static void CreateTestAssembly()
        {
            ...
            string path = Path.Combine(Application.dataPath, AutomatedQARuntimeSettings.PackageAssetsFolderName, GeneratedTestsFolderName, GeneratedTestsAssemblyName);
            if (!File.Exists(path))
        ...
        private static void GenerateAutomatedRunTest(string runPath)
        {
            CreateTestAssemblyFolder();
            CreateTestAssembly();
            CreateAutomatedRunTestScripts(runPath);
        ...
        private static void CreateAutomatedRunTestScripts(string runPath)
        {
            string content = GetAutomatedRunTestScript(runPath);
            var writePath = Path.Combine(AutomatedQARuntimeSettings.PackageAssetsFolderPath,
                GeneratedTestsFolderName,
                $"{Path.GetFileNameWithoutExtension(runPath)}_Tests.cs");
        ...
        private static void GenerateSimpleTest(string recording)
        {
        ...
        private static string GetRecordedTestScript(string recording)
        {
            ...
            var testClassName = GetClassNameForRecording(recording) + "_Tests";
        ...
        private static string GetAutomatedRunTestScript(string automatedRun)
        {
            ...
            var testClassName = runName + "_Tests";
            return templateContent
