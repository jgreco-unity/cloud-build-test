...
namespace Unity.RecordedPlayback.Editor
{
	public class SettingsWindow : EditorWindow
	{
		...
		private static SettingsWindow wnd;

		[MenuItem("Automated QA/Settings")]
		public static void ShowWindow()
		{
			wnd = GetWindow<SettingsWindow>();
			wnd.Show();
			wnd.Init();
		}
		...
		{
			initialized = false;
			wnd.titleContent = new GUIContent("Automated QA Settings");
		}
		...
		void DeleteConfigKey(string key) {
			ClearErrors();
			AutomatedQARuntimeSettings.AutomatedQASettingsData newConfig = new AutomatedQARuntimeSettings.AutomatedQASettingsData();
		...
		void LoadConfigData()
		{
			AutomatedQARuntimeSettings.AutomatedQASettingsData newConfig = AutomatedQARuntimeSettings.GetCustomSettingsData();

			for (int i = 0; i < newConfig.Configs.Count; i++)
			{
				AutomatedQARuntimeSettings.AutomationSet set = newConfig.Configs[i];
